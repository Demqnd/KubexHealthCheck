...
- `ClaudeApiSettings:ApiKey` — an Anthropic API key. Used by `POST /api/claude/ask`, and by the MCP-attached command paths (single-URL commands, `fleet`) — those need Anthropic's direct API specifically, since AWS Bedrock doesn't support the MCP connector feature at all. See "MCP-connected commands" and "Fleet reports" below.
- `ClaudeApiSettings:Model` — defaults to `claude-opus-5`.
- `BedrockSettings:ApiKey` / `BedrockSettings:Region` / `BedrockSettings:ModelId` — used by `POST /api/claude/command`'s plain (non-MCP) paths only: a bare skill like `onthisday`, or a free-form question with no URL/`fleet` prefix. `ApiKey` is a Bedrock API key (the bearer-token style, `Authorization: Bearer ...` — not an Anthropic key, not IAM credentials). `Region` is the AWS region the model is enabled in (e.g. `us-east-1`). `ModelId` is Bedrock's own model identifier (e.g. `us.anthropic.claude-opus-4-5-20251101-v1:0`), a different naming scheme than Anthropic's own model names. All three are required for this path to work; leaving them unset only breaks the plain-command path, not the MCP-attached ones.
...
**Fleet customers don't go through MCP at all.** Kubex's MCP server requires its own separately-authorized OAuth credential — explicitly confirmed in Kubex's own docs ("only users who have... explicitly authorized their LLM client application," refreshed daily) and by direct testing (a plain login token gets a real `invalid_token` rejection from the MCP server). So for each fleet customer, the backend signs in via `POST {plain-host}/api/v2/authorize` (deriving the plain host from the `mcpUrl` column by stripping `-mcp`) to get a Kubex REST API token, then fetches cluster data directly via `GET /api/v2/kubernetes/clusters` and hands that data to the model as context — no MCP server attached, and for `bedrock fleet`, no tool-use loop either, since the data's already in hand before Bedrock is even called. The single-URL `KubexMcpSettings:AuthorizationToken` path is unaffected and still uses a real MCP server, since that token is a manually-obtained one the MCP server does accept.
...
- `BEDROCK_API_KEY` / `BEDROCK_REGION` / `BEDROCK_MODEL_ID` — same as `BedrockSettings:ApiKey`/`Region`/`ModelId`. The good-morning prompt is a plain free-form question with no MCP URL, so it goes through Bedrock (see "MCP-connected commands" below) — these three need to be set or this workflow's daily run fails with a missing-config error.
